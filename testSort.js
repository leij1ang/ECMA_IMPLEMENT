...
// 如果传入的comparefn不可用，则使用默认的comparefn
		...
		while (true) {
			...
			partition: for (var i = low_end + 1; i < high_start; i++) {
				...
				if (order < 0) {
					...
					//数据变为 top_elem pivot element
					if (order < 0) {
						//如果不是等于，即a[low_end + 1]/a[i] > a[low_end] > a[high_start]top_elem
						element = a[i];
						a[i] = a[low_end];
						a[low_end] = element;
						low_end++;
						//a[low_end]  < a[low_end + 1]
					}
				}
			}
			//递归
...
var a=[{x:1},{x:0},{x:3}]
InnerArraySort(a,a.length,function (x, y) {
	return x.x < y.x
})
console.log(a)
